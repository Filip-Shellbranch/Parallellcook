...
// Coarse Mutex implementation
// Compile this benchmark by running:
// "g++ -Wall -std=c++17 -pthread -O3 coarse_mutex_bench.cpp -o bench.out"
// Run by typing:
...
int main(int argc, char *argv[])
{
    ...
    /* example use of benchmarking */
    {
        ...
        benchmark(threadcnt, u8"coarse Mutex read", [&l1](int random)
                  { read(l1, random); });
        benchmark(threadcnt, u8"coarse Mutex update", [&l1](int random)
                  { update(l1, random); });
    }
    {
        ...
        benchmark(threadcnt, u8"coarse Mutex mixed", [&l1](int random)
                  { mixed(l1, random); });
    }
    return EXIT_SUCCESS;
}
